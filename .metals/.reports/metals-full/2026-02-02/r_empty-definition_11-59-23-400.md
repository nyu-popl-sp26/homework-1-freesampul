error id: file://<WORKSPACE>/src/main/scala/popl/hw01.scala:scala/Double#
...
object hw01 extends App:
  ...
  def approx(c: Double, xn: Double): Double =
      return (((xn)^3 - c) / 3*(xn)^2)

  def approxN(c: Double, xn: Double, n: Int): Double =
    require(n >= 0)
    if n < xn then approx(c, xn - 1) else n

  def approxErr(c: Double, xn: Double, epsilon: Double): D@@ouble =
    require (epsilon > 0) 
